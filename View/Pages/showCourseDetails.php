...
if (oci_fetch($stmt)) {
    do {
        $module_id = oci_result($stmt, "MODULENUMBER");
        if (!isset($modules[$module_id])) {
            $modules[$module_id] = array(
                'name' => oci_result($stmt, "MODULENAME"),
                'lectures' => array(),
            );
        }

        $modules[$module_id]['lectures'][] = array(
            'name' => oci_result($stmt, "LECTURENAME"),
            'lectureDescription' => oci_result($stmt, "LECTUREDESCRIPTION")->load(),
            'url' => oci_result($stmt, "LECTUREVIDEOLINKS")->load(),
        );
